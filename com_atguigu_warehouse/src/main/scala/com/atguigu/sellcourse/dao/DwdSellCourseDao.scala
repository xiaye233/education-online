...
object DwdSellCourseDao {
  def getDwdSaleCourse(sparlSession:SparkSession)={
    sparlSession.sql("select courseid,coursename,status,pointlistid,majorid,chapterid,chaptername,edusubjectid," +
