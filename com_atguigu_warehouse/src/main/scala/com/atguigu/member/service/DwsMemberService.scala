...
//    result.persist(StorageLevel.MEMORY_AND_DISK_SER)
  ...
  //（DwsMember（），DwsMember（），DwsMember（））
  //(1001_webA,iterator(DwsMember(),DwsMember()....))
...
//        result.foreach(println)
  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    ...
    //查询当天增量数据
    val dayResult = sparkSession.sql(s"""
                                        |select
                                        |    a.uid,
    ...
    val reuslt = dayResult.union(historyResult)
      ...
      //(1001_webA,Iterator(MemberZipper(1001,100,银卡,2019-01-01,2019-01-02),MemberZipper(1001,300,银卡,2019-01-02,2019-01-03)))
      // ,(1002_webA,Iterator(MemberZipper(1002,100,金卡...),MemberZipper(1002,200,金卡...)))
      .mapGroups {
      case (key, iters) =>
        //val keys = key.split("_")
        //val uid = keys(0)
        ...
        if (list.size > 1 && "9999-12-31".equals(list(list.size - 2).end_time)) {
          ...
          1001,200,银卡,20190102,99991231,webA
          1001,50,银卡,20190103,99991231,webA
           */
          //获取历史数据的最后一条数据
