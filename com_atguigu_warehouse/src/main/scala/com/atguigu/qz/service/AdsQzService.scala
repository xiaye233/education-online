...
object  AdsQzService {
  ...
  def getTargetApi(sparkSession: SparkSession, dt: String) = {
    ...
    val paperPassDetail = sparkSession.sql("select * from dws.dws_user_paper_detail").cache() //下面用到多次，缓存处理下
    ...
    paperPassDetail.unpersist()  //释放缓存
    ...
    userQuestionError.join(userQuestionRight, Seq("questionid", "dn"))
      ...
      .orderBy(desc("errcount")).coalesce(3)
      .select("questionid", "errcount", "rightcount", "rate", "dt", "dn")
      .coalesce(3).write.mode(SaveMode.Append).insertInto("ads.ads_user_question_detail")
  }
