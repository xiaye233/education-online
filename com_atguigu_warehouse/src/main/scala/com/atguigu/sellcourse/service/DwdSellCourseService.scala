...
object DwdSellCourseService {
  /**
    ...
    * @param sparkSessipon
    */
  def importSaleCourseLong(ssc:SparkContext,sparkSessipon:SparkSession)={
    import sparkSessipon.implicits._
  ...
  def importCourseShoppingCart2(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/atguigu/ods/courseshoppingcart.log").filter(item=>{
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition=>{
      partition.map(item=>{
      ...
      write.partitionBy("dt","dn").bucketBy(10,"orderid").sortBy("orderid").
